...
def entity_processor_record(record: dict, record_key: str) -> List[dict]:
    ...
    # 각 텍스트 세그먼트를 기록 내 순서에 따라 변환
    for index, segment in enumerate(segments):
        d = {
            "id": 0,  # 누적 id로 업데이트됨
            "page": index + 1,  # 각 기록 내 페이지 번호
            "text": segment,
...
# FastAPI 시작 시 APScheduler를 등록하여 주기적 작업을 실행합니다.
@app.on_event("startup")
async def startup_event():
    scheduler = AsyncIOScheduler()
    # 예: 60초마다 process_chat_history 실행
    scheduler.add_job(process_chat_history, 'interval', seconds=600)
    scheduler.start()
